...
export function ProductDetail({ productId }: Readonly<{ productId: number }>) {
  ...
  const product = useQuery({
    ...
    queryFn: () => getProduct(productId, session!.access),
    enabled: Boolean(session),
  });

  useEffect(() => {
    if (product.error instanceof ApiError && product.error.status === 401) {
      logout();
      router.replace("/login");
    }
  }, [logout, product.error, router]);
  ...
  if (product.isError || !product.data) {
    return <DetailMessage title="Could not load item" message="Check the API connection and try again." />;
  }
      ...
      <div className="mt-7 grid gap-8 lg:grid-cols-[1.25fr_0.75fr]">
        <article className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 sm:p-10">
          <div className="flex items-center gap-3 text-sm text-[var(--muted)]">
            <span className="rounded-full bg-white/6 px-3 py-1 font-medium">
              {locationNames[product.data.location]}
            </span>
...
function DetailMessage({ title, message }: Readonly<{ title: string; message: string }>) {
  return (
